...
void SI4703::checkRDS(uint8_t countryPI)
{
  ...
  if (_sendRDS) {
    ...
    ///////////////////////////////////////////////
    /// Add by: J_RPM >>> countryPI RDS filter ////
    ///////////////////////////////////////////////
    bool result = false;
    // Without PI filter
    if (countryPI == 0) {
      result = true;
    // With PI filter
    }else if ((registers[RDSA] & 0xF000) >> 12 == countryPI) {
      result = true;
    }  

    // check for a RDS data set ready
    //if(registers[STATUSRSSI] & RDSR) {
    if((registers[STATUSRSSI] & RDSR) && result) {
...
void SI4703::debugStatus()
{
  ...
  for (int x = 0 ; x < 16 ; x++) {
    Serial.print(F("Reg: 0x0")); Serial.print(x, HEX);
    Serial.print(F(" = 0x")); _printHex4(registers[x]);
    Serial.println();
  } // for
} // debugStatus
